...
<form method="post" action="{{ route('book.update', $book->id) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
    ...
    <x-text-input id="title" type="text" name="title" class="mt-1 block w-full" value="{{ old('title', $book->title)}}"
        required />
    ...
    <div class="max-w-xl">
        ...
        <x-text-input id="city" type="text" name="city" class="mt-1 block w-full" value="{{ old('city', $book->city)}}"
            required />
